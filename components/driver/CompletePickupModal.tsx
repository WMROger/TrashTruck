...
export default function CompletePickupModal({
  ...
  const handleTakePhoto = async () => {
    try {
      ...
      if (locationStatus !== "granted") {
        setErrorMsg(
          "Location permission is required for Geo-Photo verification.",
        );
        return;
      }
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        try {
          ...
          console.warn(
            "Failed to get current location, trying last known:",
            locErr,
          );
          try {
            ...
            setErrorMsg(
              "Photo captured, but GPS is unavailable. Enable location services and retake the photo.",
            );
          }
        }
      }
  ...
  const handleSubmit = async () => {
    ...
    if (
      numericBagCount !== null &&
      (!Number.isInteger(numericBagCount) || numericBagCount < 0)
    ) {
    ...
    try {
      ...
      });
      {
        ...
        if (result.queued)
          Alert.alert(
            "Saved offline",
            "Completion evidence will upload automatically when internet access returns.",
          );
        // Reset states
        setImageUri(null);
        setDescription("");
        ...
        <View style={styles.modalContent}>
          ...
          >
            ...
            <View style={styles.measurementRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.label}>
                  Collected weight<Text style={{ color: "#EF4444" }}> *</Text>
                </Text>
            ...
            <Text style={styles.measurementHint}>
              {Number.isFinite(Number(measurementValue)) &&
            ...
            <View style={styles.buttonRow}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  (!imageUri || isUploading) && styles.disabledButton,
                ]}
