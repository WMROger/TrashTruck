 ...
 */
export const DICT_ADMIN_CONFIG = Object.freeze({
  primaryEmail:
    process.env.EXPO_PUBLIC_DICT_ADMIN_EMAIL || "dict@trashtrack.gov.ph",
  knownEmails: [
    process.env.EXPO_PUBLIC_DICT_ADMIN_EMAIL || "dict@trashtrack.gov.ph",
 ...
 */
export async function ensureDictProfileInFirestore(
  uid: string,
  email: string,
  displayName?: string,
) {
  ...
  try {
    ...
    if (!snap.exists() || snap.data()?.role !== "dict") {
      console.log(
        "Auto-provisioning / healing DICT profile in Firestore for UID:",
        uid,
      );
      await setDoc(
        userRef,
 ...
 */
export async function loginOrBootstrapDictAccount(
  ...
  try {
    ...
    await ensureDictProfileInFirestore(
      credential.user.uid,
      email,
      credential.user.displayName || DICT_ADMIN_CONFIG.displayName,
    );
    ...
    if (isAccountMissing || isDefaultPassword) {
      ...
      try {
        ...
        await updateProfile(newCred.user, {
          displayName: DICT_ADMIN_CONFIG.displayName,
        });
        await ensureDictProfileInFirestore(
          newCred.user.uid,
          email,
          DICT_ADMIN_CONFIG.displayName,
        );
        return newCred.user;
      } catch (createErr: any) {
        if (createErr.code === "auth/email-already-in-use") {
          ...
          if (!isDefaultPassword) {
            const fallbackCred = await signInWithEmailAndPassword(
              auth,
              email,
              DICT_ADMIN_CONFIG.defaultPassword,
            );
